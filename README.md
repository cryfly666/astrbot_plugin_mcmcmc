...
### 3. 初始化消息发送
**重要**: 在监控能够发送消息到群之前，需要先在目标群内使用任意指令（如 `/查询` 或 `/start_server_monitor`）来初始化消息来源。这样插件才能记录消息发送的目标位置。
...
- ⚠️ **首次使用或监控无法发送消息时，请先在目标群内使用任意指令（如 `/查询`）来初始化消息来源**
...
**Q: 消息没有发送到群里？**  
A: 
1. 首先确保已在目标群内使用过任意指令（如 `/查询` 或 `/start_server_monitor`）来初始化消息来源
2. 检查日志中是否有 "unified_msg_origin 未设置" 的警告
3. 检查群号是否正确，机器人是否有发送权限
4. 如果使用 `enable_auto_monitor` 自动启动，在启动监控前先在群内使用 `/查询` 指令进行初始化
